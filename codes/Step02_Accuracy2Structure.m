...
if strcmp(iitt, 'cross')
    savefile_location = [file_location ProjectName '/Results/total/CROSS/mat/'];
end
...
for i_subject = 7:19
    ...
    if strcmp(iitt, 'cross')
        mat_location = [file_location ProjectName '/Results/' SubjectName '/mat/CROSS/'];
    end
...
%                 Result.Between.matrix = Between;
                ...
                if(speeds{i_speed} == 1)
                    load([mat_location 'Cross_train_1_test_2.mat']);
                    Result.param = param_train;
                    Result.param.train_speed = param_train.speed;
                    Result.param.test_speed = param_test.speed;
                else
                    load([mat_location 'Cross_train_2_test_1.mat']);
                    Result.param = param_test;
                    Result.param.train_speed = param_train.speed;
                    Result.param.test_speed = param_test.speed;
                end
                
                Result.param.iitt = 'cross';
                ...
                if(speeds{i_speed} == 1)
                    ...
                    save([savefile_location 'CROSS_' SubjectName '_train_2_test_1.mat'], 'Result');
                end
                clear Result;
            end
        end
